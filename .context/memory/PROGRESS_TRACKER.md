...
| Affiliation P6-D0 | ✅ **MERGÉ** (PR #40, head `1e8aa79`, merge `dcdc966`, CI SUCCESS ; suite complète **1524 / 11835**, P6-D0 **63 / 2546**, Pint **495**) — **FONDATION BDD SEULE** (D-057, migration **000029**, **45 migrations**, aucune `000030`). **9 tables** : `affiliate_program_policies`, `affiliates`, `affiliate_codes`, `affiliate_touches`, `affiliate_attributions`, `affiliate_commissions`, `affiliate_commission_entries`, `affiliate_payouts`, `affiliate_payout_items`. **Politiques versionnées, jamais rétroactives** (index unique partiel : **une seule politique `active`**) ; **taux en `INTEGER` bps borné 0..5000**, snapshoté sur chaque commission ; **commission au niveau `order_item`**, base **`line_total_after_discount`** seule autorisée ; **ledger append-only** (trigger `BEFORE UPDATE OR DELETE` ⇒ `23514`), montants **signés** avec direction contrainte par type ; **payout mono-affilié, mono-devise, manuel**, **aucun provider**, **aucune donnée bancaire/Mobile Money**. `affiliates.user_id` **UNIQUE** (D-014 : jamais un `users.role`). `visitor_id` est un **ancrage d'identité**, pas une preuve : les colonnes `first_touch_*` et `events` restent **non autoritatives** (D-037). ACL fail-closed : **runtime sans aucun droit**, PUBLIC sans accès, **aucun nouveau rôle**, **aucune fonction `SECURITY DEFINER` opérationnelle** (différées à P6-D1/D2/D3). **Aucune politique insérée** par la migration. **Durcissement pré-merge** : ancrage de touche par **trigger `BEFORE INSERT`** et non par CHECK (un CHECK vetoerait toute purge de visiteur définitivement, car `ON DELETE SET NULL` le réévalue) ; **politique effective physiquement immuable** (trigger `BEFORE UPDATE`) ; **FK composites** rendant structurelles les cohérences commission↔commande↔ligne↔attribution↔affilié, ainsi que **payout mono-affilié ET mono-devise** ; **idempotence naturelle** (un seul `accrual` par commission, un seul `refund_reversal` par `(commission, refund)`). Un seul objet hors bloc : l'index `order_items (id, order_id)`, **créé par `000029` et retiré par son `down()`** — aucune migration historique modifiée. Aucune extension PostgreSQL. ⚠️ **FONDATION DORMANTE** : aucun flux de candidature, d'attribution, de commission, de payout, aucune route, aucun service, aucun job, aucun écran. |
...
| Découpage | ✅ P6-A0 → ✅ A1.0 attribution → ✅ A1.1 autorité rollup → ✅ A1.2 worker/réconciliation → ✅ A1.3 backfill explicite → ✅ A2 segments → ✅ B0 vues → ✅ B1 exports → ✅ C paniers/relances → **✅ D0 fondation affiliation** → D1 politique + identité + codes → D2 touches + attribution → D3 moteur commissions + compensations → D4 payout administratif → D5 surfaces admin |
| Vues admin CRM P6-B0 | ✅ **MERGÉ** (PR #37, head `b05edb2`, merge `2df7e6f`, CI SUCCESS ; suite complète B0 standalone **1295 / 8250**) — D-052, migration **000026**. **7 autorités de lecture** `STABLE`+`SECURITY DEFINER` (owner `digitrove_crm_executor`, runtime EXECUTE-only, aucun nouveau rôle) — D-051 avait annoncé « aucune migration », l'audit a prouvé qu'aucune autorité contact/consentement/commerce/membership/versions n'existait. Verticales Contacts et Segments, constructeur de critères **structuré** (aucun textarea/JSON/SQL), e-mail **exact normalisé dans l'autorité**, anonymisé ⇒ `email IS NULL` irrécupérable, **aucun total multi-devises**, **aucune division par 100**, aucune route publique, aucun envoi, aucun export. **PostgreSQL reste l'autorité finale du DSL** (prouvé hors builder : 7 définitions invalides refusées, 0 version créée). ⚠️ **Aucune suite complète locale B0 exécutée ni revendiquée** — différée au stack B1. P6B0+P6B01 **113 tests / 655 assertions**, Pint **446 fichiers**. |
| Exports P6-B1 | ✅ **MERGÉ** (PR #38, head `bf9ea09`, merge `474f92c`, CI SUCCESS ; suite complète **1379 / 8711**) — D-053 → D-054, migration **000027**, **43 migrations**. Table `crm_exports` + **10 autorités** `SECURITY DEFINER` (runtime EXECUTE-only, ni `SELECT` ni DML, aucun nouveau rôle) ; `down()` restaure exactement `000026`. **Génération figée à la création** (une G2 publiée pendant l'écriture n'apparaît pas) ; plafond strict `row_limit + 1` ⇒ `failed`/`row_limit_exceeded` **sans fichier**, jamais de troncature ; **sûreté formule CSV** (`= + - @ TAB CR LF`, DDE et `=HYPERLINK` prouvés) ; **aucun I/O sous transaction** ; téléchargement **propriétaire seul** avec **404 plat** indistinguable ; job **ID-only** ; flags et schedulers **off par défaut**. ⚠️ Défaut d'autorisation réel fermé : `parent::canAccess()` contournait la Gate (PHP aplatit les traits) — remplacé par un hook `crmGateExtraCondition()`. B0+B1 **197 tests / 1112 assertions**, Pint **463 fichiers**. |
...
| Affiliation P6-D1.1 | ⬜ **NON COMMENCÉ** — cycle de vie affilié (candidature, validation/refus, activation, suspension, fermeture) + codes. ⚠️ La **re-candidature après `rejected`** est différée à son préflight : `affiliates.user_id` est **UNIQUE**, donc le schéma impose une transition d'état, pas une seconde ligne. |
| Prochain gate | ✅ P6-B0 (#37), P6-B1 (#38), P6-C (#39) et **P6-D0** (#40, merge `dcdc966`) **MERGÉS**. **P6-D1 implémenté et validé localement, en PR non mergée** (`000030`, 46 migrations). **PROCHAIN GATE : P6-D1.1 — cycle de vie affilié + codes**, NON COMMENCÉ, aucune migration `000031`. ⚠️ La **re-candidature après `rejected`** est différée à son préflight (`affiliates.user_id` est UNIQUE ⇒ transition d'état, jamais une seconde ligne). Le schéma P6-D est complet ; D-057 fige les frontières D1→D5 et **interdit d'anticiper une autorité**. ⚠️ Dettes ouvertes : **D-030 GLOBAL** (P4-C `assertMailerSafe()` plus faible sur cinq points, à durcir dans son propre gate) ; **absence de flux panier applicatif** (P6-C dormant) ; **absence de storefront** (P6-D0 dormant — aucun clic réel à attribuer) ; **`AGENTS.md` gate P6-D0 non exposé côté payout** (aucune donnée Mobile Money : un gate dédié reste requis avant tout versement réel). |
